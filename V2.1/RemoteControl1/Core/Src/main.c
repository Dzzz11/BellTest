...
uint8_t wakeup_bag[4] = {0x20,0x01,0x01,0x61};//0x20：包头，0x01：一号遥控器�???0x01：一号功能表示唤醒，0x61：包�???
uint8_t ChosePlayer1[4] = {0x20,0x01,0x02,0x61};//0x20：包头，0x01：一号遥控器�???0x02：二号功能表示�?�择角色1�???0x61：包�???
uint8_t ChosePlayer2[4] = {0x20,0x01,0x03,0x61};//0x20：包头，0x01：一号遥控器�??? 0x03：三号功能表示�?�择角色2�???0x61：包�???
uint8_t Left[4] = {0x20,0x01,0x04,0x61};//0x20：包头，0x01：一号遥控器�??? 0x04：四号功能表示左移，0x61：包�???
uint8_t Jump[4] = {0x20,0x01,0x05,0x61};//0x20：包头，0x01：一号遥控器�??? 0x05：五号功能表示跳跃，0x61：包�???
uint8_t Right[4] = {0x20,0x01,0x06,0x61};//0x20：包头，0x01：一号遥控器�??? 0x06：六号功能表示又移，0x61：包�???
uint8_t ResetBag[4] = {0x20,0x01,0x00,0x61};
...
int main(void)
{
  ...
  {
    if (ChosePlayer1Flag ) {
      while ( AnswerBuffer[0] != 0x20 && AnswerBuffer[1] != 0x01 && AnswerBuffer[2] != 0x02 && AnswerBuffer[3] != 0x61) {
    ...
    if (ChosePlayer2Flag) {
      while ( AnswerBuffer[0] != 0x20 && AnswerBuffer[1] != 0x01 && AnswerBuffer[2] != 0x03 && AnswerBuffer[3] != 0x61) {
    ...
    if (HAL_GPIO_ReadPin(GPIOB,GPIO_PIN_12) == GPIO_PIN_RESET || MPU6050Data.roll > 15) {
      HAL_Delay(50);
      if (HAL_GPIO_ReadPin(GPIOB,GPIO_PIN_12) == GPIO_PIN_RESET || MPU6050Data.roll > 15) {
        HAL_UART_Transmit(&huart1,Left,sizeof(Left),100);
      }
    }
    if (HAL_GPIO_ReadPin(GPIOB,GPIO_PIN_13) == GPIO_PIN_RESET || MPU6050Data.pitch < -15) {
      HAL_Delay(50);
      if (HAL_GPIO_ReadPin(GPIOB,GPIO_PIN_13) == GPIO_PIN_RESET || MPU6050Data.pitch < -15) {
        HAL_UART_Transmit(&huart1,Jump,sizeof(Jump),100);
      }
    }
    if (HAL_GPIO_ReadPin(GPIOB,GPIO_PIN_14) == GPIO_PIN_RESET || MPU6050Data.roll < -15) {
      HAL_Delay(50);
      if (HAL_GPIO_ReadPin(GPIOB,GPIO_PIN_14) == GPIO_PIN_RESET || MPU6050Data.roll < -15) {
