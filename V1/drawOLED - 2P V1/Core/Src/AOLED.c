...
/***************************************************************************************
  ...
  * 
  * 程序名称：				0.96寸AOLED显示屏驱动程序（4针脚I2C接口）
  * 程序创建时间：			2023.10.24
  * 当前程序版本：			V2.0
  * 当前版本发布时间：		2024.10.20
  * 
  ...
  * 程序介绍及更新动态：	jiangxiekeji.com/tutorial/AOLED.html
  * 
...
/**
  ...
  * 随后调用AOLED_Update函数或AOLED_UpdateArea函数
  * 才会将显存数组的数据发送到AOLED硬件，进行显示
  */
uint8_t AOLED_DisplayBuf[8][128];
...
/**
  ...
  */
void AOLED_W_SCL(uint8_t BitValue)
{
...
/**
  ...
  */
void AOLED_W_SDA(uint8_t BitValue)
{
...
/**
  ...
  */
void AOLED_GPIO_Init(void)
{
	uint32_t i, j;
	
	/*在初始化前，加入适量延时，待AOLED供电稳定*/
	for (i = 0; i < 1000; i ++)
	{
		for (j = 0; j < 1000; j ++);
	}
...
/**
  ...
  * 返 回 值：无
  */
void AOLED_I2C_Start(void)
{
	AOLED_W_SDA(1);		//释放SDA，确保SDA为高电平
	AOLED_W_SCL(1);		//释放SCL，确保SCL为高电平
	AOLED_W_SDA(0);		//在SCL高电平期间，拉低SDA，产生起始信号
	AOLED_W_SCL(0);		//起始后把SCL也拉低，即为了占用总线，也为了方便总线时序的拼接
}
...
/**
  ...
  * 返 回 值：无
  */
void AOLED_I2C_Stop(void)
{
	AOLED_W_SDA(0);		//拉低SDA，确保SDA为低电平
	AOLED_W_SCL(1);		//释放SCL，使SCL呈现高电平
	AOLED_W_SDA(1);		//在SCL高电平期间，释放SDA，产生终止信号
}
...
/**
  ...
  * 返 回 值：无
  */
void AOLED_I2C_SendByte(uint8_t Byte)
{
	uint8_t i;
	...
	for (i = 0; i < 8; i++)
	{
		...
		/*两个!的作用是，让所有非零的值变为1*/
		AOLED_W_SDA(!!(Byte & (0x80 >> i)));
		AOLED_W_SCL(1);	//释放SCL，从机在SCL高电平期间读取SDA
		AOLED_W_SCL(0);	//拉低SCL，主机开始发送下一位数据
	}
	
	AOLED_W_SCL(1);		//额外的一个时钟，不处理应答信号
	AOLED_W_SCL(0);
}
...
{
	AOLED_I2C_Start();				//I2C起始
	AOLED_I2C_SendByte(0x78);		//发送AOLED的I2C从机地址
	AOLED_I2C_SendByte(0x00);		//控制字节，给0x00，表示即将写命令
	AOLED_I2C_SendByte(Command);		//写入指定的命令
	AOLED_I2C_Stop();				//I2C终止
}
...
/**
  ...
  * 返 回 值：无
  */
void AOLED_WriteData(uint8_t *Data, uint8_t Count)
{
	uint8_t i;
	
	AOLED_I2C_Start();				//I2C起始
	AOLED_I2C_SendByte(0x78);		//发送AOLED的I2C从机地址
	AOLED_I2C_SendByte(0x40);		//控制字节，给0x40，表示即将写数据
	/*循环Count次，进行连续的数据写入*/
	for (i = 0; i < Count; i ++)
	{
		AOLED_I2C_SendByte(Data[i]);	//依次发送Data的每一个数据
	}
	AOLED_I2C_Stop();				//I2C终止
}
...
{
	AOLED_GPIO_Init();			//先调用底层的端口初始化
	
	/*写入一系列的命令，对AOLED进行初始化配置*/
	AOLED_WriteCommand(0xAE);	//设置显示开启/关闭，0xAE关闭，0xAF开启
	
	AOLED_WriteCommand(0xD5);	//设置显示时钟分频比/振荡器频率
	AOLED_WriteCommand(0x80);	//0x00~0xFF
	
	AOLED_WriteCommand(0xA8);	//设置多路复用率
	AOLED_WriteCommand(0x3F);	//0x0E~0x3F
	
	AOLED_WriteCommand(0xD3);	//设置显示偏移
	AOLED_WriteCommand(0x00);	//0x00~0x7F
	
	AOLED_WriteCommand(0x40);	//设置显示开始行，0x40~0x7F
	
	AOLED_WriteCommand(0xA1);	//设置左右方向，0xA1正常，0xA0左右反置
	
	AOLED_WriteCommand(0xC8);	//设置上下方向，0xC8正常，0xC0上下反置

	AOLED_WriteCommand(0xDA);	//设置COM引脚硬件配置
	AOLED_WriteCommand(0x12);
	
	AOLED_WriteCommand(0x81);	//设置对比度
	AOLED_WriteCommand(0xCF);	//0x00~0xFF

	AOLED_WriteCommand(0xD9);	//设置预充电周期
	AOLED_WriteCommand(0xF1);

	AOLED_WriteCommand(0xDB);	//设置VCOMH取消选择级别
	AOLED_WriteCommand(0x30);

	AOLED_WriteCommand(0xA4);	//设置整个显示打开/关闭

	AOLED_WriteCommand(0xA6);	//设置正常/反色显示，0xA6正常，0xA7反色

	AOLED_WriteCommand(0x8D);	//设置充电泵
	AOLED_WriteCommand(0x14);

	AOLED_WriteCommand(0xAF);	//开启显示
	
	AOLED_Clear();				//清空显存数组
	AOLED_Update();				//更新显示，清屏，防止初始化后未显示内容时花屏
}
...
/**
  ...
  * 返 回 值：无
  * 说    明：AOLED默认的Y轴，只能8个Bit为一组写入，即1页等于8个Y轴坐标
  */
void AOLED_SetCursor(uint8_t Page, uint8_t X)
{
	/*如果使用此程序驱动1.3寸的AOLED显示屏，则需要解除此注释*/
	/*因为1.3寸的AOLED驱动芯片（SH1106）有132列*/
	/*屏幕的起始列接在了第2列，而不是第0列*/
	/*所以需要将X加2，才能正常显示*/
//	X += 2;
	
	/*通过指令设置页地址和列地址*/
	AOLED_WriteCommand(0xB0 | Page);					//设置页位置
	AOLED_WriteCommand(0x10 | ((X & 0xF0) >> 4));	//设置X位置高4位
	AOLED_WriteCommand(0x00 | (X & 0x0F));			//设置X位置低4位
}
...
/**
  ...
  * 返 回 值：等于X的Y次方
  */
uint32_t AOLED_Pow(uint32_t X, uint32_t Y)
{
...
/**
  ...
  */
uint8_t AOLED_pnpoly(uint8_t nvert, int16_t *vertx, int16_t *verty, int16_t testx, int16_t testy)
{
	int16_t i, j, c = 0;
...
/**
  ...
  */
uint8_t AOLED_IsInAngle(int16_t X, int16_t Y, int16_t StartAngle, int16_t EndAngle)
{
	int16_t PointAngle;
...
/**
  * 函    数：将AOLED显存数组更新到AOLED屏幕
  * 参    数：无
  * 返 回 值：无
  * 说    明：所有的显示函数，都只是对AOLED显存数组进行读写
  *           随后调用AOLED_Update函数或AOLED_UpdateArea函数
  *           才会将显存数组的数据发送到AOLED硬件，进行显示
...
void AOLED_Update(void)
{
	...
	for (j = 0; j < 8; j ++)
	{
		/*设置光标位置为每一页的第一列*/
		AOLED_SetCursor(j, 0);
		/*连续写入128个数据，将显存数组的数据写入到AOLED硬件*/
		AOLED_WriteData(AOLED_DisplayBuf[j], 128);
	}
}

/**
  * 函    数：将AOLED显存数组部分更新到AOLED屏幕
...
void AOLED_UpdateArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
	...
	for (j = Page; j < Page1; j ++)
	{
		if (X >= 0 && X <= 127 && j >= 0 && j <= 7)		//超出屏幕的内容不显示
		{
			/*设置光标位置为相关页的指定列*/
			AOLED_SetCursor(j, X);
			/*连续写入Width个数据，将显存数组的数据写入到AOLED硬件*/
			AOLED_WriteData(&AOLED_DisplayBuf[j][X], Width);
		}
	}
}
...
void AOLED_Clear(void)
{
	...
	for (j = 0; j < 8; j ++)				//遍历8页
	{
		...
		{
			AOLED_DisplayBuf[j][i] = 0x00;	//将显存数组数据全部清零
		}
	}
}
...
void AOLED_ClearArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
	...
	for (j = Y; j < Y + Height; j ++)		//遍历指定页
	{
		for (i = X; i < X + Width; i ++)	//遍历指定列
		{
			...
			{
				AOLED_DisplayBuf[j / 8][i] &= ~(0x01 << (j % 8));	//将显存数组指定数据清零
			}
		}
	}
}
...
void AOLED_Reverse(void)
{
	...
	for (j = 0; j < 8; j ++)				//遍历8页
	{
		for (i = 0; i < 128; i ++)			//遍历128列
		{
			AOLED_DisplayBuf[j][i] ^= 0xFF;	//将显存数组数据全部取反
		}
	}
}
...
void AOLED_ReverseArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
	...
	for (j = Y; j < Y + Height; j ++)		//遍历指定页
	{
		for (i = X; i < X + Width; i ++)	//遍历指定列
		{
			if (i >= 0 && i <= 127 && j >=0 && j <= 63)			//超出屏幕的内容不显示
			{
				AOLED_DisplayBuf[j / 8][i] ^= 0x01 << (j % 8);	//将显存数组指定数据取反
			}
		}
	}
}
...
{
	if (FontSize == AOLED_8X16)		//字体为宽8像素，高16像素
	{
		/*将ASCII字模库AOLED_F8x16的指定数据以8*16的图像格式显示*/
		AOLED_ShowImage(X, Y, 8, 16, OLED_F8x16[Char - ' ']);
	}
	else if(FontSize == AOLED_6X8)	//字体为宽6像素，高8像素
	{
		/*将ASCII字模库AOLED_F6x8的指定数据以6*8的图像格式显示*/
		AOLED_ShowImage(X, Y, 6, 8, OLED_F6x8[Char - ' ']);
	}
}

/**
  * 函    数：AOLED显示字符串（支持ASCII码和中文混合写入）
  ...
  *                 AOLED_6X8		宽6像素，高8像素
  * 返 回 值：无
  * 说    明：显示的中文字符需要在AOLED_Data.c里的AOLED_CF16x16数组定义
  *           未找到指定中文字符时，会显示默认图形（一个方框，内部一个问号）
  *           当字体大小为AOLED_8X16时，中文字符以16*16点阵正常显示
  *           当字体大小为AOLED_6X8时，中文字符以6*8点阵显示'?'
  * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
void AOLED_ShowString(int16_t X, int16_t Y, char *String, uint8_t FontSize)
{
	...
	while (String[i] != '\0')	//遍历字符串
	{
		...
		{
			/*使用AOLED_ShowChar显示此字符*/
			AOLED_ShowChar(X + XOffset, Y, SingleChar[0], FontSize);
			XOffset += FontSize;
		}
		else					//否则，即多字节字符
		{
			...
			if (FontSize == AOLED_8X16)		//给定字体为8*16点阵
			{
				/*将字模库AOLED_CF16x16的指定数据以16*16的图像格式显示*/
				AOLED_ShowImage(X + XOffset, Y, 16, 16, OLED_CF16x16[pIndex].Data);
				XOffset += 16;
			}
			else if (FontSize == AOLED_6X8)	//给定字体为6*8点阵
			{
				/*空间不足，此位置显示'?'*/
				AOLED_ShowChar(X + XOffset, Y, '?', AOLED_6X8);
				XOffset += AOLED_6X8;
			}
		}
	}
...
void AOLED_ShowNum(int16_t X, int16_t Y, uint32_t Number, uint8_t Length, uint8_t FontSize)
{
	...
	{
		/*调用AOLED_ShowChar函数，依次显示每个数字*/
		/*Number / AOLED_Pow(10, Length - i - 1) % 10 可以十进制提取数字的每一位*/
		/*+ '0' 可将数字转换为字符格式*/
		AOLED_ShowChar(X + i * FontSize, Y, Number / AOLED_Pow(10, Length - i - 1) % 10 + '0', FontSize);
	}
}

/**
  * 函    数：AOLED显示有符号数字（十进制，整数）
  * 参    数：X 指定数字左上角的横坐标，范围：-32768~32767，屏幕区域：0~127
  * 参    数：Y 指定数字左上角的纵坐标，范围：-32768~32767，屏幕区域：0~63
...
void AOLED_ShowSignedNum(int16_t X, int16_t Y, int32_t Number, uint8_t Length, uint8_t FontSize)
{
	...
	if (Number >= 0)						//数字大于等于0
	{
		AOLED_ShowChar(X, Y, '+', FontSize);	//显示+号
	...
	{
		/*调用AOLED_ShowChar函数，依次显示每个数字*/
		/*Number1 / AOLED_Pow(10, Length - i - 1) % 10 可以十进制提取数字的每一位*/
		/*+ '0' 可将数字转换为字符格式*/
		AOLED_ShowChar(X + (i + 1) * FontSize, Y, Number1 / AOLED_Pow(10, Length - i - 1) % 10 + '0', FontSize);
	}
}

/**
  * 函    数：AOLED显示十六进制数字（十六进制，正整数）
  * 参    数：X 指定数字左上角的横坐标，范围：-32768~32767，屏幕区域：0~127
  * 参    数：Y 指定数字左上角的纵坐标，范围：-32768~32767，屏幕区域：0~63
...
void AOLED_ShowHexNum(int16_t X, int16_t Y, uint32_t Number, uint8_t Length, uint8_t FontSize)
{
	...
	{
		/*以十六进制提取数字的每一位*/
		SingleNumber = Number / AOLED_Pow(16, Length - i - 1) % 16;
		
		if (SingleNumber < 10)			//单个数字小于10
		{
			/*调用AOLED_ShowChar函数，显示此数字*/
			/*+ '0' 可将数字转换为字符格式*/
			AOLED_ShowChar(X + i * FontSize, Y, SingleNumber + '0', FontSize);
		}
		else							//单个数字大于10
		{
			/*调用AOLED_ShowChar函数，显示此数字*/
			/*+ 'A' 可将数字转换为从A开始的十六进制字符*/
			AOLED_ShowChar(X + i * FontSize, Y, SingleNumber - 10 + 'A', FontSize);
		}
	}
}

/**
  * 函    数：AOLED显示二进制数字（二进制，正整数）
  * 参    数：X 指定数字左上角的横坐标，范围：-32768~32767，屏幕区域：0~127
  * 参    数：Y 指定数字左上角的纵坐标，范围：-32768~32767，屏幕区域：0~63
  * 参    数：Number 指定要显示的数字，范围：0x00000000~0xFFFFFFFF
...
void AOLED_ShowBinNum(int16_t X, int16_t Y, uint32_t Number, uint8_t Length, uint8_t FontSize)
{
	...
	{
		/*调用AOLED_ShowChar函数，依次显示每个数字*/
		/*Number / AOLED_Pow(2, Length - i - 1) % 2 可以二进制提取数字的每一位*/
		/*+ '0' 可将数字转换为字符格式*/
		AOLED_ShowChar(X + i * FontSize, Y, Number / AOLED_Pow(2, Length - i - 1) % 2 + '0', FontSize);
	}
}

/**
  * 函    数：AOLED显示浮点数字（十进制，小数）
  * 参    数：X 指定数字左上角的横坐标，范围：-32768~32767，屏幕区域：0~127
  * 参    数：Y 指定数字左上角的纵坐标，范围：-32768~32767，屏幕区域：0~63
...
void AOLED_ShowFloatNum(int16_t X, int16_t Y, double Number, uint8_t IntLength, uint8_t FraLength, uint8_t FontSize)
{
	...
	PowNum = AOLED_Pow(10, FraLength);		//根据指定小数的位数，确定乘数
	FraNum = round(Number * PowNum);		//将小数乘到整数，同时四舍五入，避免显示误差
	IntNum += FraNum / PowNum;				//若四舍五入造成了进位，则需要再加给整数
	
	/*显示整数部分*/
	AOLED_ShowNum(X + FontSize, Y, IntNum, IntLength, FontSize);
	
	/*显示小数点*/
	AOLED_ShowChar(X + (IntLength + 1) * FontSize, Y, '.', FontSize);
	
	/*显示小数部分*/
	AOLED_ShowNum(X + (IntLength + 2) * FontSize, Y, FraNum, FraLength, FontSize);
}
...
void AOLED_ShowImage(int16_t X, int16_t Y, uint8_t Width, uint8_t Height, const uint8_t *Image)
{
	...
	/*将图像所在区域清空*/
	AOLED_ClearArea(X, Y, Width, Height);
	...
	for (j = 0; j < (Height - 1) / 8 + 1; j ++)
	{
		...
		for (i = 0; i < Width; i ++)
		{
			if (X + i >= 0 && X + i <= 127)		//超出屏幕的内容不显示
			{
				...
				{
					/*显示图像在当前页的内容*/
					AOLED_DisplayBuf[Page + j][X + i] |= Image[j * Width + i] << (Shift);
				}
				
				if (Page + j + 1 >= 0 && Page + j + 1 <= 7)		//超出屏幕的内容不显示
				{					
					/*显示图像在下一页的内容*/
					AOLED_DisplayBuf[Page + j + 1][X + i] |= Image[j * Width + i] >> (8 - Shift);
				}
			}
		}
	}
...
/**
  * 函    数：AOLED使用printf函数打印格式化字符串（支持ASCII码和中文混合写入）
...
void AOLED_Printf(int16_t X, int16_t Y, uint8_t FontSize, char *format, ...)
{
	...
	va_end(arg);							//结束变量arg
	AOLED_ShowString(X, Y, String, FontSize);//AOLED显示字符数组（字符串）
}
...
void AOLED_DrawPoint(int16_t X, int16_t Y)
{
	...
	{
		/*将显存数组指定位置的一个Bit数据置1*/
		AOLED_DisplayBuf[Y / 8][X] |= 0x01 << (Y % 8);
	}
}
...
uint8_t AOLED_GetPoint(int16_t X, int16_t Y)
{
	...
	{
		/*判断指定位置的数据*/
		if (AOLED_DisplayBuf[Y / 8][X] & 0x01 << (Y % 8))
		{
			return 1;	//为1，返回1
		}
	}
...
void AOLED_DrawLine(int16_t X0, int16_t Y0, int16_t X1, int16_t Y1)
{
	...
	else				//斜线
	{
		...
		/*画起始点，同时判断标志位，将坐标换回来*/
		if (yflag && xyflag){AOLED_DrawPoint(y, -x);}
		else if (yflag)		{AOLED_DrawPoint(x, -y);}
		else if (xyflag)	{AOLED_DrawPoint(y, x);}
		else				{AOLED_DrawPoint(x, y);}
...
void AOLED_DrawRectangle(int16_t X, int16_t Y, uint8_t Width, uint8_t Height, uint8_t IsFilled)
{
	...
	if (!IsFilled)		//指定矩形不填充
	{
		...
		{
			AOLED_DrawPoint(i, Y);
			AOLED_DrawPoint(i, Y + Height - 1);
		}
		/*遍历左右Y坐标，画矩形左右两条线*/
		for (i = Y; i < Y + Height; i ++)
		{
			AOLED_DrawPoint(X, i);
			AOLED_DrawPoint(X + Width - 1, i);
		}
	}
	else				//指定矩形填充
	{
...
void AOLED_DrawTriangle(int16_t X0, int16_t Y0, int16_t X1, int16_t Y1, int16_t X2, int16_t Y2, uint8_t IsFilled)
{
	...
	{
		/*调用画线函数，将三个点用直线连接*/
		AOLED_DrawLine(X0, Y0, X1, Y1);
		AOLED_DrawLine(X0, Y0, X2, Y2);
		AOLED_DrawLine(X1, Y1, X2, Y2);
	}
	else					//指定三角形填充
	{
		/*找到三个点最小的X、Y坐标*/
		...
		for (i = minx; i <= maxx; i ++)
		{
			...
			for (j = miny; j <= maxy; j ++)
			{
				/*调用AOLED_pnpoly，判断指定点是否在指定三角形之中*/
				/*如果在，则画点，如果不在，则不做处理*/
				if (AOLED_pnpoly(3, vx, vy, i, j)) {AOLED_DrawPoint(i, j);}
			}
		}
	}
}
...
void AOLED_DrawCircle(int16_t X, int16_t Y, uint8_t Radius, uint8_t IsFilled)
{
	...
	/*画每个八分之一圆弧的起始点*/
	AOLED_DrawPoint(X + x, Y + y);
	AOLED_DrawPoint(X - x, Y - y);
	AOLED_DrawPoint(X + y, Y + x);
	AOLED_DrawPoint(X - y, Y - x);
...
void AOLED_DrawArc(int16_t X, int16_t Y, uint8_t Radius, int16_t StartAngle, int16_t EndAngle, uint8_t IsFilled)
{
	...
	if (AOLED_IsInAngle(x, y, StartAngle, EndAngle))	{AOLED_DrawPoint(X + x, Y + y);}
	if (AOLED_IsInAngle(-x, -y, StartAngle, EndAngle)) {AOLED_DrawPoint(X - x, Y - y);}
	if (AOLED_IsInAngle(y, x, StartAngle, EndAngle)) {AOLED_DrawPoint(X + y, Y + x);}
	if (AOLED_IsInAngle(-y, -x, StartAngle, EndAngle)) {AOLED_DrawPoint(X - y, Y - x);}
	...
	if (IsFilled)	//指定圆弧填充
	{
		...
		for (j = -y; j < y; j ++)
		{
			...
			if (AOLED_IsInAngle(0, j, StartAngle, EndAngle)) {AOLED_DrawPoint(X, Y + j);}
		}
	}
	...
	while (x < y)		//遍历X轴的每个点
	{
		...
		if (IsFilled)	//指定圆弧填充
		{
			...
			{
				/*在填充圆的每个点时，判断指定点是否在指定角度内，在，则画点，不在，则不做处理*/
				if (AOLED_IsInAngle(x, j, StartAngle, EndAngle)) {AOLED_DrawPoint(X + x, Y + j);}
				if (AOLED_IsInAngle(-x, j, StartAngle, EndAngle)) {AOLED_DrawPoint(X - x, Y + j);}
			}
			...
			{
				/*在填充圆的每个点时，判断指定点是否在指定角度内，在，则画点，不在，则不做处理*/
				if (AOLED_IsInAngle(-y, j, StartAngle, EndAngle)) {AOLED_DrawPoint(X - y, Y + j);}
				if (AOLED_IsInAngle(y, j, StartAngle, EndAngle)) {AOLED_DrawPoint(X + y, Y + j);}
			}
		}
	}
